"""
Regenerate visualizations for affected luxury looks after fixes.
"""
import asyncio
import base64
import sys
...
async def generate_visualization_for_look(look_id: int, google_ai: GoogleAIStudioService):
    ...
    with get_db_session() as db:
        ...
        for p in products_data:
            print(f"  - [{p.get('product_type')}] {p['name'][:50]}...")
        ...
        try:
            result = await google_ai.generate_add_multiple_visualization(
                ...
                existing_products=[],
                workflow_id=f"regen-{look_id}-{datetime.now().timestamp()}",
            )
...
async def main():
    ...
    # Affected looks after fixes
    affected_looks = [172, 175, 177, 179, 182, 183, 185, 186, 187, 188]

    print(f"Regenerating {len(affected_looks)} affected looks")
    print(f"Start: {datetime.now()}")

    success = 0
    failed = []

    for look_id in affected_looks:
        try:
    ...
    print(f"\n{'='*60}")
    print(f"Done! Success: {success}/{len(affected_looks)}")
    if failed:

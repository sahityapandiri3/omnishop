...
RESPOND WITH JSON ONLY - NO OTHER TEXT."""
    ...
    async def remove_furniture(self, image_base64: str, max_retries: int = 5) -> Optional[str]:
        ...
        try:
            ...
            prompt = """You are an expert at removing furniture from room images while preserving the room's appearance.

TASK: Remove all moveable furniture and objects from this room. The room must look empty and ready for new furniture.

MUST REMOVE:
- All furniture: sofas, chairs, tables, beds, storage units, shelves, desks
- Electronics: TVs, monitors, speakers
- Moveable lighting: floor lamps, table lamps
- Rugs and carpets (unless wall-to-wall installed)
- Plants and planters
- Wall art, mirrors, frames
- Decorative objects
- Curtains and window treatments

MUST PRESERVE (do NOT change these):
- Walls: keep exact color, texture, and any moldings
- Floors: same material visible where furniture was
- Ceiling and fixed lighting (recessed lights, built-in fixtures)
- Windows and doors
- Built-in architectural elements
- Room perspective, angle, and dimensions
- Natural lighting and shadows

CRITICAL REQUIREMENTS:
1. The room must look like the SAME room, just empty
2. Do NOT change wall colors or floor colors
3. Do NOT change the room shape or perspective
4. Fill areas where furniture was removed with matching floor/wall
5. Result should look photorealistic and natural

OUTPUT: A photorealistic image of the EXACT SAME ROOM but empty - same walls, same floor, same lighting, just no furniture."""

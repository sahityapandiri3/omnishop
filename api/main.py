...
# Temporary endpoint to initialize stores table
@app.post("/init-stores")
async def init_stores():
    """One-time endpoint to create stores table and populate with all stores including new mid-tier ones"""
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not table_exists:
            ...
            """))
            await session.execute(text("CREATE INDEX ix_stores_name ON stores (name)"))
            await session.execute(text("CREATE INDEX ix_stores_budget_tier ON stores (budget_tier)"))
            await session.execute(text("CREATE INDEX ix_stores_is_active ON stores (is_active)"))
            await session.commit()

        # Insert stores (skip if already exists)
        now = datetime.utcnow()
        inserted = 0
        ...
        return {
            "status": "success",
            "message": f"Stores table initialized. Inserted {inserted} new stores.",
            "total_stores": total,

...
# Temporary endpoint to migrate Look 19 to production
@app.post("/migrate-look-19")
async def migrate_look_19():
    """One-time endpoint to migrate Look 19 from local to production"""
    ...
    async with AsyncSessionLocal() as session:
        ...
        if existing:
            return {"status": "exists", "message": f"Look already exists with ID {existing.id}"}

        # Load images from scripts directory
        import os
        ...
        return {
            "status": "success",
            "message": f"Created look with ID {look.id}",
            "look_id": look.id,
            "products": len(PRODUCT_IDS),
            "total_price": total_price,
            "has_room_image": room_image is not None,
            "has_viz_image": viz_image is not None,
        }

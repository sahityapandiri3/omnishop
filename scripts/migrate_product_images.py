"""
Script to migrate product_images from local PostgreSQL to Railway PostgreSQL
...
def migrate_product_images(source_url: str, target_url: str, batch_size: int = 100):
    """
    Migrate product_images from source to target database
    ...
    """
    logger.info("Starting product_images migration...")
    ...
    try:
        ...
        if remaining == 0:
            logger.info("\n✅ All product_images already migrated!")
            return
        ...
        logger.info("Finding which images are missing...")
        missing_ids_result = source_session.execute(text("""
        ...
        # Fallback: Get IDs from source and target, then compare
        logger.info("Getting all image IDs from source...")
        source_ids_result = source_session.execute(
            text("SELECT id FROM product_images ORDER BY id")
        )
        source_ids = set(row[0] for row in source_ids_result)

        logger.info("Getting all image IDs from target...")
        ...
        logger.info(f"Found {len(missing_ids)} missing images to migrate")

        if not missing_ids:
            logger.info("✅ All images already migrated!")
            return

        logger.info(f"\nMigrating {len(missing_ids)} product_images in batches of {batch_size}...")
        ...
        with tqdm(total=len(missing_ids), desc="Migrating missing images") as pbar:
                ...
                try:
                    ...
                    for row in rows:
                        ...
                        try:
                            ...
                            logger.error(f"Error inserting image {row_dict.get('id')}: {e}")
                            errors += 1
                    ...
                    if (i // batch_size) % 10 == 0 and total_migrated > 0:
                        logger.info(f"Migrated {total_migrated}/{len(missing_ids)} images")
        ...
        try:
            max_id_result = target_session.execute(
                text("SELECT MAX(id) FROM product_images")
            )
            max_id = max_id_result.scalar()
            if max_id:
                target_session.execute(
                    text("SELECT setval('product_images_id_seq', :max_id)"),
                    {"max_id": max_id}
                )
                target_session.commit()
                logger.info(f"Updated product_images_id_seq to {max_id}")
        except Exception as e:
        ...
        if final_count == source_count:
            logger.info("\n✅ All product_images successfully migrated!")
        else:
            missing = source_count - final_count
            logger.warning(f"\n⚠️  {missing} images still missing. Run the script again to retry.")
...
def main():
    ...
    if not target_url:
        ...
        logger.info("  python scripts/migrate_product_images.py")
        sys.exit(1)

    # Confirm migration
    logger.info("\n" + "="*60)
    logger.info("PRODUCT IMAGES MIGRATION")
    logger.info("="*60)
    ...
    logger.info("\nThis will migrate product_images with timeout handling.")
    ...
    migrate_product_images(source_url, target_url, batch_size=batch_size)

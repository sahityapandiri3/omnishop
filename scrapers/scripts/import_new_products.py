...
Handles new scraped products from Urban Ladder, Home Center, Wooden Street v2
Deduplication: source_website + external_id
"""
import json
import re
import sys
...
EXCLUDED_CATEGORIES = ['Clock', 'Table Clock', 'Wall Clock']
...
def get_or_create_category(session, category_name: str):
    ...
    # Check if slug already exists (e.g., "Rug" and "Carpet" might both generate similar slugs)
    existing_slug = session.query(Category).filter_by(slug=slug).first()
    if existing_slug:
        # Return the existing category with that slug
...
def import_product(session, product_data: dict, source_website: str):
    ...
    # Check if product already exists (deduplication by source_website + external_id)
    ...
    # Handle source_url (different field names in scraped data)
    product.source_url = product_data.get('source_url') or product_data.get('product_url')
    ...
    # Handle images (support both image_urls array and single image_url)
    image_urls = product_data.get('image_urls', [])
    single_image = product_data.get('image_url')
    if single_image and not image_urls:
...
def import_json_file(json_path: str, source_website: str, display_name: str, exclude_categories: list = None):
    ...
    if exclude_categories:
        ...
        products = [p for p in products if p.get('category') not in exclude_categories]
        filtered_count = original_count - len(products)
        if filtered_count > 0:
            logger.info(f"Filtered out {filtered_count} products from excluded categories: {exclude_categories}")
    ...
    with get_db_session() as session:
        ...
        for i, product_data in enumerate(products):
            try:
                result = import_product(session, product_data, source_website)
                if result[0] is None:
                    error_count += 1
                    continue

                product, was_existing = result
...
def main():
    ...
    import_files = [
        {
            'path': '/tmp/urbanladder_products_v2.json',
            'source_website': 'urbanladder',
            'display_name': 'Urban Ladder',
            'exclude_categories': []
        },
        {
            'path': '/tmp/homecentre_products.json',
            'source_website': 'homecentre',
            'display_name': 'Home Center',
            'exclude_categories': []
        },
        {
            'path': '/tmp/woodenstreet_products_v2.json',
            'source_website': 'woodenstreet',
            'display_name': 'Wooden Street',
            'exclude_categories': EXCLUDED_CATEGORIES  # Exclude clocks
        }
    ]
    ...
    for file_config in import_files:
        if Path(file_config['path']).exists():
            new, updated, errors = import_json_file(
                ...
                file_config.get('exclude_categories', [])
            )
            total_new += new

"""
Playwright-based scraper for Pepperfry - Fixed v3
Uses correct DOM selectors: .product-card-container
"""
import asyncio
import json
import hashlib
...
async def scroll_and_collect_products(page: Page, max_scrolls: int = 15) -> List[str]:
    """Scroll and collect product URLs from product cards"""
    product_urls = set()
    prev_count = 0
    no_change_count = 0

    for i in range(max_scrolls):
        # Find product cards using class selectors
        cards = await page.query_selector_all('.product-card-container, .clip-product-card-wrapper')

        for card in cards:
            # Look for anchor tags inside the card
            ...
            for link in links:
                ...
                if href and '/product/' in href and '.html' in href:
                    ...
                    break  # Only get first product link per card
...
async def main():
        ...
        for group_name, categories in CATEGORY_URLS.items():
            ...
            for cat_url, category in categories:
                ...
                try:
                    ...
                    # Scrape each product (limit to 100 per category)
    ...
    # Save results
    output_file = '/tmp/pepperfry_products_v3.json'
